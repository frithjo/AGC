...
const Command: React.FC<CommandProps> = ({ type, onCommand, ...props }) => {
  useEffect(() => {
    if (onCommand) {
      onCommand({ type, ...props });
    }
  }, [type, onCommand, props]);
...
const components = (onCommand?: (props: any) => void) => ({
  ...
  command: ({ type, ...props }: CommandProps) => (
    <Command type={type} onCommand={onCommand} {...props} />
  ),
...
export const Markdown: React.FC<MarkdownProps> = ({ children, onCommand }) => {
  return (
    <ReactMarkdown
      remarkPlugins={[remarkGfm]}
      rehypePlugins={[rehypeRaw, rehypeMinifyWhitespace]}
      components={components(onCommand)}
    >
      {children}
    </ReactMarkdown>
  );
